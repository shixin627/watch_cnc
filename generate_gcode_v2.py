...
def calc_arc_z(x, r, z_layer, xc=XC, zc=ZC):
    ...
    Arc equation: Z = Zc - sqrt(R^2 - (X-Xc)^2)
    Returns: min(Z_arc, Z_layer) to not cut deeper than current layer
    """
    dx = x - xc
...
def calc_x_range(r, z_layer, xc=XC, zc=ZC):
    """
    Calculate valid X range for current layer
    Based on: X_range = sqrt(R^2 - (Zc - Z_layer)^2)
    """
    dz = zc - z_layer
    ...
    x_range = math.sqrt(discriminant)
    return xc - x_range, xc + x_range
...
def generate_gcode(zigzag=True):
    ...
    output.append("%")
    output.append("; " + "=" * 60)
    output.append("; 2.5D Arc Surface Constant-Thickness Scanning")
    output.append("; ")
